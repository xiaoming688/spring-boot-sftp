...
public class RestUploadController {
    ...
    public ResponseEntity<byte[]> testDownload(@RequestBody FileDownloadVO vo) throws IOException {
        File file = sftpService.downloadFile("/sclp/orig/article/" + vo.getFileName(), "D:/upload/" + vo.getFileName());
    ...
    @RequestMapping(value = "/api/downloadFiles")
    public String downloadFiles(@RequestBody FileDownloadVO vo) throws IOException {
        File file = sftpService.downloadFile("/sclp/orig/article/" + vo.getFileName(), "D:/upload/" + vo.getFileName());
        HttpHeaders headers = new HttpHeaders();
        headers.setContentDispositionFormData("attachment", vo.getFileName());
        headers.setContentType(MediaType.APPLICATION_OCTET_STREAM);
